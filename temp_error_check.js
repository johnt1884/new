        ...
        function generateShortcutScript() {
            ...
            if (hasEditorChanges) {
                // If there are editor changes, we'll generate a script that handles both shortcuts and video processing
                // For simplicity and since the user asked for edit/s to be carried out when script is generated
                // we will integrate FFmpeg logic.
            }
            ...
            let scriptLines = [
                ...
                '    \$points.Add(0)',
                '    \$points.Add(\$duration)',
                '    foreach (\$m in \$markers) { \$points.Add(\$m.time) }',
                '    foreach (\$c in \$cuts) { \$points.Add(\$c.start); \$points.Add(\$c.end) }',
                '    ',
                ...
                '        \$mid = (\$start + \$end) / 2',
                '        \$isCut = \$false',
                '        foreach (\$c in \$cuts) { if (\$mid -ge \$c.start -and \$mid -lt \$c.end) { \$isCut = \$true; break } }',
                '        if (\$isCut) { continue }',
                '        ',
                '        \$rotation = 0',
                '        foreach (\$m in \$markers) { if (\$start -ge \$m.time) { \$rotation = \$m.rotation } }',
                '        ',
                ...
                '        ffmpeg -y -i "\$sourcePath" -ss \$seg.start.ToString("F3") -t \$dur -vf "\$vf" -c:v libx264 -crf 17 -preset veryslow -c:a copy -avoid_negative_ts make_zero "\$outSeg" 2>\$null',
                '        (echo "file \'\$outSeg\'") | Out-File -FilePath "\$listPath" -Append -Encoding utf8',
                '    }',
                '    ',
                '    ffmpeg -y -f concat -safe 0 -i "\$listPath" -c copy "\$targetPath" 2>\$null',
            ...
            selectionsToProcess.forEach(sel => {
                ...
                if (hasEdits) {
                    ...
                    if (sel.markers.length === 0 && sel.rotation && sel.rotation !== "0") {
                ...
                if (selectionType === 'subfolder-sc' || selectionType === 'both-sc') {
                        ...
                        if (projectPathToUse) {
                            scDirPath = `Join-Path -Path (Join-Path -Path \"$BaseDir\" -ChildPath '${projectPathToUse}') -ChildPath 'sc'`;
                        } else {
