...
(function () {
    ...
    function handleIntersection(entries, observer) {
        entries.forEach(entry => {
                ...
                if (isLoaded) {
                    if (embedType === 'twitter') { // Twitter-specific unloading logic
                        ...
                        if (innerContainer) {
                            ...
                            let linkText = `View Tweet (ID: ${placeholder.dataset.tweetId}) 🐦 (Scroll to load)`;
                            try {
                                const urlObj = new URL(originalUrl);
                                const pathParts = urlObj.pathname.split('/');
                                if (pathParts.length > 2 && pathParts[2] === 'status') {
                                   linkText = `View Tweet by @${pathParts[1]} (ID: ${placeholder.dataset.tweetId}) 🐦 (Scroll to load)`;
                                }
                            } catch (e) { /* ignore, use default linkText */ }
                            innerContainer.innerHTML = `<a href="${originalUrl}" target="_blank" rel="noopener noreferrer" style="color: #1da1f2; text-decoration: none;">${linkText}</a>`;
                        }
                        placeholder.dataset.loaded = 'false';
                        // placeholder.style.minHeight = '250px'; // Restore min-height if changed
            ...
            if (embedType === 'twitter') {
                const innerContainer = placeholder.querySelector('.tweet-content-container');
                if (!innerContainer) {
                    console.error('[OTK Viewer IO] Twitter outer placeholder has no inner container.', placeholder);
                    return; // Skip this entry
                }

                if (entry.isIntersecting) {
                    if (!isLoaded) {
                        console.log(`[OTK Viewer IO] Loading Tweet: ${placeholder.dataset.tweetId}`);
                        innerContainer.innerHTML = 'Loading Tweet...'; // Update inner container text
                        ensureTwitterWidgetsLoaded().then(() => {
                            createTweetWithTimeout(placeholder.dataset.tweetId, innerContainer, {
                                theme: 'light',
                                conversation: 'none',
                                align: 'center',
                                width: 500,
                                ...
                                placeholder.dataset.loaded = 'true';
                                // Optional: Adjust outer placeholder style if needed after load
                                // placeholder.style.minHeight = 'auto';
                            }).catch(() => {
                                // Error handling is inside createTweetWithTimeout, which updates innerContainer.
                                placeholder.dataset.loaded = 'false'; // Or true if error message is the "loaded" state
                            });
                        }).catch(error => {
                            console.error('[OTK Viewer IO] Failed to load Twitter widgets for tweet.', error);
                            innerContainer.innerHTML = `<a href="${placeholder.dataset.originalUrl}" target="_blank" rel="noopener noreferrer" style="color: #1da1f2; text-decoration: none;">Error loading widgets. View Tweet (ID: ${placeholder.dataset.tweetId}) 🐦</a>`;
                        });
                    }
                }
                // Unloading for Twitter is handled in the "else" block for entry.isIntersecting above
    ...
    function createTwitterEmbedPlaceholder(tweetId, originalUrl) {
        ...
        // The outer div needs 'embed-placeholder' class for the IntersectionObserver to pick it up
        return `
        <div class="embed-placeholder tweet-outer-placeholder" data-embed-type="twitter" data-tweet-id="${tweetId}" data-original-url="${escapedOriginalUrl}" style="min-height: 250px; display: flex; align-items: center; justify-content: center; border: 1px solid #ccc; margin: 5px 0; background-color: #f0f0f0; padding: 10px;">
            <div class="tweet-content-container" id="tweet-embed-placeholder-${tweetId}-${randomSuffix}" style="width: 100%; text-align: center;">
                <a href="${escapedOriginalUrl}" target="_blank" rel="noopener noreferrer" style="color: #1da1f2; text-decoration: none;">View Tweet (ID: ${tweetId}) 🐦 (Scroll to load)</a>
            </div>
        </div>`.trim();
    }
...
async function manageInitialScroll() {
    ...
    function convertQuotes(text, embedCounts) {
        ...
        text = text.replace(twitterRegexG, (match, originalUrl, tweetId) => {
            ...
            return createTwitterEmbedPlaceholder(tweetId, originalUrl); // Call the updated function
        });
...
.tweet-content-container {
    ...
    async function renderAllMessages() { // Ensure it's async (already was)
        ...
        const currentPlaceholders = viewer.querySelectorAll('.embed-placeholder, .tweet-outer-placeholder'); // Ensure both are selected
